...
export default class Resolver extends EventEmitter {
  ...
  injectDeps(className, modules) {
    ...
    if (this.singletons[className]) return wrapSingleton.call(this, className)
    ...
    switch (item.type) {
      case "module": // Single module
        ...
        if (module.singleton) {
          this.singletons[className] = new (module.default.bindArray(resolvedDeps))
          return wrapSingleton.call(this, className)
        }
